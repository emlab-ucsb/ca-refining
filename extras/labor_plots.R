...
## refactor
dac_df$scenario_title <- factor(dac_df$scenario_title, levels = c('BAU demand\nhistoric production',
                                                                        'BAU demand\nhistoric exports',
                                                                        'BAU demand\nlow exports',
                                                                        'Low demand\nhistoric exports',
                                                                        'Low demand\nlow exports',
...
## refactor
dac_df$scenario <- factor(dac_df$scenario, levels = c('BAU demand - historic production',
...
dac_fig <- ggplot(dac_df %>% filter(!scenario %in% remove_scen,
                                    demo_cat == "DAC"), aes(x = year, y = demo_emp / 1000, lty = title, color = county)) +
...
dac_gaps_df$scenario <- factor(dac_gaps_df$scenario, levels = c('BAU demand - historic production',
                                                            'BAU demand - historic exports',
                                                            'BAU demand - low exports',
                                                            'Low demand - historic exports',
                                                            'Low demand - low exports',
...
dac_gaps_df$scenario_title <- factor(dac_gaps_df$scenario_title, levels = c('BAU demand\nhistoric production',
                                                                        'BAU demand\nhistoric exports',
                                                                        'BAU demand\nlow exports',
                                                                        'Low demand\nhistoric exports',
                                                                        'Low demand\nlow exports',
...
## fig
dac_gap_fig_b <- ggplot(dac_gaps_df %>% filter(!scenario %in% remove_scen,
                                               demo_cat == "DAC"), aes(x = year, y = gap / 1000, lty = title, color = county)) +
  geom_line(linewidth = 0.8, alpha = 0.7) +
  geom_hline(yintercept = 0, color = "darkgray", linewidth = 0.5) +
  facet_grid(demo_cat ~ scenario_title) +
  labs(x = NULL,
