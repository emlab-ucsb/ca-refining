...
list(
  ...
  tar_target(
    ...
    command = simple_ggsave(fig_demand_ghg,
      main_path,
      "outputs/academic-out/refining/cuf=0.8/figures/2024-08-update",
  ...
  tar_target(
    name = save_ct_xwalk,
    command = simple_fwrite(ct_xwalk, main_path, "outputs/refining-2024/cuf=0.8/health", "ct_xwalk_2019_2020.csv"),
    format = "file"
  ),
  tar_target(
    name = save_health_income,
    command = simple_fwrite(refining_health_income, main_path, "outputs/refining-2024/cuf=0.8/health", "refining_health_income_2023.csv"),
    format = "file"
  ),
  tar_target(
    name = save_health_income_2000,
    command = simple_fwrite(health_weighted, main_path, "outputs/refining-2024/cuf=0.8/health", "refining_health_census_tract.csv"),
    format = "file"
  ),
  tar_target(
    name = save_mortality,
    command = simple_fwrite(refining_mortality, main_path, "outputs/refining-2024/cuf=0.8/health", "refining_mortality_2023.csv"),
    format = "file"
  ),
  tar_target(
    name = save_state_mort_levels,
    command = simple_fwrite(ref_mort_level, main_path, "outputs/refining-2024/cuf=0.8/health", "refining_state_mortality.csv"),
    format = "file"
  ),
