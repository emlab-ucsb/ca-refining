...
marathon_carson <- filter(df,
                          w_geocode == 60379800021006) %>%
  summarize(SI01 = sum(SI01), 
            S000 = sum(S000))
summary(marathon_carson$SI01)
summary(marathon_carson$S000)

chevron_elsegundo <- filter(df,
                              w_geocode == 60379800301000) %>%
  summarize(SI01 = sum(SI01), 
            S000 = sum(S000))
summary(chevron_elsegundo$SI01)
summary(chevron_elsegundo$S000)

valero_benicia <- filter(df,
                            w_geocode == 60952521022025) %>%
  summarize(SI01 = sum(SI01), 
            S000 = sum(S000))
summary(valero_benicia$SI01)
summary(valero_benicia$S000)
