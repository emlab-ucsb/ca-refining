...
get_intrastate_jet_forecast <- function(raw_intra_jet) {
  dt_intra <- melt(raw_intra_jet,
    measure.vars = colnames(raw_intra_jet)[2:35],
...
get_its_forecast <- function(raw_its_bau, raw_its_lc1, raw_avgas) {
  ...
  bau_long <- melt(raw_its_bau,
    measure.vars = as.character(2017:2050),
    variable.name = "year", value.name = "consumption_million_gge"
  )
  ...
  avgas_long <- melt(raw_avgas,
    measure.vars = colnames(raw_avgas)[2:35],
    variable.name = "year", value.name = "consumption_million_gge"
  )
  ...
  bau_adj <- rbindlist(
    list(
      bau_long[, .(year, fuel, scenario, consumption_gge)],
  ...
  lc1_adj <- rbindlist(
    list(
      lc1_long[, .(year, fuel, scenario, consumption_gge)],
