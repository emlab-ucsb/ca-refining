...
state_fuel_demand_df <- function(dt_its,
  ...
  dt_jet_adj <- rbindlist(list(
    copy(jet_mid)[, adj_scenario := "BAU"],
  ...
  demand_state[, fuel := factor(fuel, c(
    "gasoline", "diesel", "jet",
...
create_time_series_exports <- function(refined_movements_annual, region_fuel_ratio, ei_crude, ei_gasoline, ei_diesel, ei_jet) {
  ...
  # get long form
  ts_exports <- melt(ts_exports_wide,
    id.vars = c("year"), measure.vars = c("gasoline", "diesel", "jet"),
