...
calc_labor_outputs <- function(
  ...
  write_csv(
    review_df,
    file.path(save_path, "fig-csv-files", "labor_result_for_review.csv")
  )
...
calc_labor_outputs_x_impact <- function(
  main_path,
  save_path,
  proc_labor_dest_df,
  indiv_prod_output,
  ...
  write_csv(
    ...
    file.path(
      save_path,
      "fig-csv-files",
      "labor_result_x_impact_type_for_review.csv"
    )
  )
  ...
  write_csv(
    county_out_labor,
    file.path(save_path, "fig-csv-files", "labor_result_x_impact_type.csv")
  )
...
calculate_labor_x_demg_annual <- function(
  ...
  c_ct_df <- merge(c_ct_df, ca_regions, by = c("county"), allow.cartesian = T)
...
calculate_annual_labor_x_demg_hl <- function(
  ...
  fwrite(
    labor_pct_long,
    file.path(save_path, "fig-csv-files", "labor_high_low_annual_outputs.csv")
  )
...
calc_county_level_outputs <- function(
  ...
  county_region_ratio <- merge(pop_2020, ca_regions, by = "county")
  ...
  county_region_ratio <- county_region_ratio[, .(
    county,
    region,
    county_pop,
    region_pop,
  ...
  labor_county_region_out <- merge(
    labor_county_region_out,
    county_region_ratio,
    by = "region",
    allow.cartesian = T
  )
  ...
  labor_county_region_out <- labor_county_region_out %>%
    ...
    select(
      ...
      demo_emp_h_county,
      demo_emp_l_county,
      demo_comp_PV_l_county,
      demo_comp_PV_h_county
    ) %>%
    pivot_longer(
      demo_emp_h_county:demo_comp_PV_h_county,
      names_to = "metric",
      values_to = "value"
    ) %>%
    group_by(
