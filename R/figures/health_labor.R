...
plot_npv_health_labor <- function(main_path,
  ...
  ## refactor
  plot_df_health$scenario <- factor(plot_df_health$scenario, levels = c('BAU demand - historic production',
  ...
  plot_df_health[, unit := fifelse(metric %in% c("avoided_health_cost_ghg", "forgone_wages_ghg"), 
  ...
  ## refactor
  plot_df_labor$scenario <- factor(plot_df_labor$scenario, levels = c('BAU demand - historic production',
                                                                        'BAU demand - historic exports',
                                                                        'BAU demand - low exports',
                                                                        'Low demand - historic exports',
                                                                        'Low demand - low exports',
  ...
  plot_df_labor[, unit := fifelse(metric %in% c("avoided_health_cost_ghg", "forgone_wages_ghg"),
                                   "NPV per avoided GHG MtCO2e\n(2019 USD million / MtCO2e)",
                                   "NPV (2019 USD billion)")]
  ...
  hist_prod = as.data.table(plot_df_health %>% filter(scen_id == bau_scen,
  ...
  fig_bxm_a <- ggplot() +
    ...
    geom_point(data = plot_df_health %>% filter(!scen_id %in% remove_scen,
...
plot_health_levels <- function(main_path,
  ...
  health_level_fig_b <- ggplot(fig2_df %>% filter(!scen_id %in% remove_scen,
                                                  demo_cat == "DAC"), aes(x = year, y = mortality_level_dem, lty = title)) +
    geom_line(linewidth = 1, alpha = 0.8) +
    geom_hline(yintercept = 0, color = "darkgray", linewidth = 0.5) +
    facet_grid(demo_cat ~ scenario_title) +
    labs(x = NULL,
...
plot_health_levels_pc <- function(main_path,
  ...
  ## refactor
  mort_pc_df$scenario_title <- factor(mort_pc_df$scenario_title, levels = c('BAU demand\nhistorical production',
                                                                      'BAU demand\nhistorical exports', 
                                                                      'BAU demand\nlow exports', 
                                                                      'Low demand\nhistorical exports',
                                                                      'Low demand\nlow exports',
  ...
  ## refactor
  mort_pc_df$scenario <- factor(mort_pc_df$scenario, levels = c('BAU demand - historical production',
                                                          'BAU demand - historical exports', 
                                                          'BAU demand - low exports', 
                                                          'Low demand - historical exports',
                                                          'Low demand - low exports',
  ...
  health_level_fig_a <- ggplot(mort_pc_df %>% filter(!scen_id %in% remove_scen,
                                                  title %in% fig_title_vec,
                                                  demo_cat == "Race")  %>%
                                 mutate(title = factor(title, levels = c("Black", "Hispanic", "Asian", "white"))), 
  ...
  ##
  health_level_fig_b <- ggplot(mort_pc_df %>% filter(!scen_id %in% remove_scen,
                                                  demo_cat == "DAC"), aes(x = year, y = value_pmil, lty = title)) +
    geom_line(linewidth = 1, alpha = 0.8) +
    geom_hline(yintercept = 0, color = "darkgray", linewidth = 0.5) +
    facet_grid(demo_cat ~ scenario_title) +
    labs(x = NULL,
  ...
  ##
  health_level_fig_c <- ggplot(mort_pc_df %>% filter(!scen_id %in% remove_scen,
                                                  demo_cat == "Poverty") %>%
                                 mutate(title = factor(title, levels = c("Below poverty line", "Above poverty line"))), 
...
plot_hl_levels_df <- function(main_path,
   ...
   ## merge with health and ghg
   health_labor_ghg_df <- merge(health_ghg_df, labor_df[, .(demand_scenario, refining_scenario, demo_cat, demo_group, title, sum_demo_comp_pv_h, sum_demo_comp_pv_l, 
  ...
  plot_df_long <- copy(demographic_npv_df)
   ...
   labor_level_fig_b <- ggplot() +
     geom_point(data = plot_df_long %>% filter(!scen_id %in% remove_scen,
                ...
                color = "black", size = 3, alpha = 0.8) +
     scale_shape_manual(values = poverty_ptc_l,
   ...
   labor_level_fig_c <- ggplot() +
     geom_point(data = plot_df_long %>% filter(!scen_id %in% remove_scen,
                                               demo_cat == "DAC",
                                               segment == "labor"), aes(x = scen_title, y = value / 1e9, shape = demo_grp_metric),
                color = "black", size = 3, alpha = 0.8) +
     geom_hline(yintercept = 0, color = "darkgray", linewidth = 0.5) +
     scale_shape_manual(values = dac_hl_ptc,
                        labels = dac_hl_labs) +
     facet_wrap(~seg_title) +
     labs(y = "NPV (USD billion)",
          x = NULL,
...
plot_hl_levels_pc <- function(demographic_npv_df,
  ...
  labor_level_fig_c <- ggplot() +
    geom_point(data = plot_df_long %>% filter(!scen_id %in% remove_scen,
                                              demo_cat == "DAC",
                                              segment == "labor"), aes(x = scen_title, y = value, shape = demo_grp_metric),
               color = "black", size = 3, alpha = 0.8) +
    geom_hline(yintercept = 0, color = "darkgray", linewidth = 0.5) +
    scale_shape_manual(values = dac_hl_ptc,
                       labels = dac_hl_labs) +
...
plot_hl_shares <- function(main_path,
  ...
  labor_share_fig_a <- ggplot() +
    ...
    geom_point(data = share_df %>% filter(!scen_id %in% remove_scen,
               ...
               size = 3, alpha = 0.8) +
    facet_wrap(~seg_title) +
    scale_color_manual(name = "with re-emp:",
                       values = race_col_pal) +
    scale_shape_manual(name = "",
                      values = race_shape_ptc,
                      labels = high_low_labs) +
  ...
  legend_fig_labor_h <- ggplot() +
    ...
    geom_point(data = share_df %>% filter(!scen_id %in% remove_scen,
               ...
               size = 3, alpha = 0.8) +
    facet_wrap(~seg_title) +
    scale_color_manual(name = "no re-emp:",
                       values = race_col_pal) +
    scale_shape_manual(name = "",
                       values = race_shape_ptc,
                       labels = high_low_labs) +
  ...
  labor_share_fig_b <- ggplot() +
    geom_point(data = share_df %>% filter(!scen_id %in% remove_scen,
                                              ...
                                              segment == "labor") %>%
                 mutate(title = factor(title, levels = c("Below poverty line", "Above poverty line")),
                        demo_grp_metric = factor(demo_grp_metric, levels = c("Above poverty line_forgone_wages_l",
               ...
               color = "black", size = 3, alpha = 0.8) +
    scale_shape_manual(values = poverty_pt_share_l,
  ...
  labor_share_fig_c <- ggplot() +
    geom_point(data = share_df %>% 
                 filter(!scen_id %in% remove_scen,
                        demo_cat == "DAC",
                        segment == "labor") %>%
               mutate(demo_grp_metric = factor(demo_grp_metric, levels = c("DAC_forgone_wages_l",
                                                                    "Non-DAC_forgone_wages_l",
                                                                    "DAC_forgone_wages_h",
                                                                    "Non-DAC_forgone_wages_h"))), aes(x = scen_title, y = share, shape = demo_grp_metric),
               color = "black", size = 3, alpha = 0.8) +
    geom_hline(yintercept = 0, color = "darkgray", linewidth = 0.5) +
  ...
  ggsave(plot = health_dac_legend,
         device ="png",
         filename = "health_dac_legend.png",
  ...
  ggsave(plot = health_poverty_legend,
         device ="png",
         filename = "health_poverty_legend.png",
         path = paste0(main_path, "outputs/academic-out/refining/figures/2022-12-update/legends/"),
         dpi = 600)
  ...
  ggsave(plot = health_race_legend,
         device ="png",
         filename = "health_race_legend.png",
         path = paste0(main_path, "outputs/academic-out/refining/figures/2022-12-update/legends/"),
         dpi = 600)
  ...
  ggsave(plot = labor_poverty_legend,
         device ="png",
         filename = "labor_poverty_legend.png",
         path = paste0(main_path, "outputs/academic-out/refining/figures/2022-12-update/legends/"),
         dpi = 600)
  ...
  ggsave(plot = labor_race_legend,
         device ="png",
         filename = "labor_race_legend.png",
         path = paste0(main_path, "outputs/academic-out/refining/figures/2022-12-update/legends/"),
         dpi = 600)
