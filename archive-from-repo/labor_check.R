...
df.agg <- group_by(
  ...
  summarize(
    total_comp_usd19_h = sum(total_comp_usd19_h, na.rm = TRUE),
...
df.bau <- filter(
  ...
  rename(
    total_comp_usd19_h_bau = total_comp_usd19_h,
...
df.agg <- left_join(df.agg, df.bau, by = c("year")) %>%
  mutate(
    total_comp_usd19_h = total_comp_usd19_h - total_comp_usd19_h_bau,
